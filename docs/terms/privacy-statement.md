...
5.  进行相关的推广活动；
6.  进行数据分析、研究和开发与我们服务的使用相关的统计信息；
7.  储存并维护与您相关的信息，用于我们的业务运营或履行法律义务；
8.  提供基于位置的服务；
9.  发送通知；
10. 改善用户体验；
11. 其他征得您同意的目的；
...
您可以在盯梢应用中点击“我的”、“账号设置”、“删除账号”开始删除账号。确认后，系统将在 24 小时内删除您的账号信息，在此期间，您可以在应用相同位置取消删除。
...
- | 处理信息类型                                                                                                                  | 目的和用途                                                                                                    |
  | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
  | 用户操作后，获取 APP 提供的用户已选择图片                                                                                     | 用于实现分享与收藏功能，将用户选择的图片分享到微信朋友圈或微信收藏，但不会用于关联和追踪用户                  |
  | 用户授权后，向 APP 提供用户选择的微信头像、昵称                                                                               | 用于实现微信登录功能                                                                                          |
  | 用户发起支付后，获取 APP 提供的微信支付订单标识并传输到本设备上的微信 APP；用户完成支付后，获取支付是否成功的状态并传输到 APP | 用于实现在 APP 中使用微信支付功能，跳转到微信 APP 相关页面以便用户进行支付操作，并向 APP 反馈本次支付是否成功 |
  | （仅限 Android 版）当前设备上是否已安装微信                                                                                   | 用于确认该设备上是否能实现微信 Open SDK 的相关能力，未安装微信 APP 的设备无法使用上述能力                     |

### 推送 SDK（仅 Android 客户端）

以下 SDK 均用于在移动终端设备进行消息推送。

#### 极光推送 SDK（仅部分 Android 客户端）
...
- 收集个人数据类型：设备信息、应用的基本信息
- SDK 数据安全说明：https://developer.hihonor.com/cn/kitdoc?category=base&kitId=11002&navigation=guides&docId=sdk-data-security.md

#### 小米推送 SDK

- 收集个人数据类型：设备标识(OAID、加密的 Android ID)、推送消息内容、设备信息(设备厂商、型号、归属地、运营商名称等)、通知栏设置
